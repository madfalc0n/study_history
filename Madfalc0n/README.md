...
1. 재택 근무
2. PhotonKit GUI 기능 제작



## 2021-10-21

1. PhotonKit GUI 기능 제작 전 코드 아키텍처 설계
2. 고객사 요청테스트 진행
...
1. Code Form 회의 및 POC 관련 보고서 작성
2. 백신 맞을 각오 다짐
...
1. 논문 읽기
2. AI training UI 버그기능 수정
3. 코드관련 미팅
4. Qt creator tutorial 진행
...
1. AD 및 continual learning 논문 읽기 
2. 초분광 카메라 하드웨어 연동 테스트 진행
   1. 트러블 슈팅 완료..버전 문제
3. 테스트 PC에서 AI Taining UI 테스트진행
   1. 성공....버그 수정 필요
...
1. BAE, BCAE 모델 설계 및 성능 테스트
   - bad...fck....
   - 새로운 loss 및 학습방법 개선필요


## 2021-08-24

1. BAE 모델 설계 및 성능 테스트
   - bad...
...
1. CapsNET, MCC 논문 읽고 loss 정의 및 모델링 설계
...
1. AD 관련 방법론을 찾기 위한 논문 탐색



## 2021-08-13

1. 고객사 POC, good 실전에 강한 놈이었음



## 2021-08-12

1. AD 최종 테스트 및 고객사 POC 시나리오 점검, 새로운 모델의 결과가 좋지 않음ㅠ
...
1. AD 모델 성능 테스트
2. 특허명세 구체화.....진보성이 없음? 새로운 아이디어 시도해볼 필요 있다고 판단
...
1. 농식품 박람회 촬영
2. AD 성능 테스트 및 포팅 테스트



## 2021-08-02

1. AD 성능 테스트ㅣ나ㅓㅇ리너이ㅏㄹㄴㅇgood



## 2021-08-01

1. AD 관련 논문 읽기 및 성능테스트
...
2. 기술이전 관련 화상미팅
3. 세미나 참석
4. AD 성능 테스트 진행 및 보고서 작성

## 2021-07-22
1. AD 성능 테스트 진행 및 보고서 작성
...
## 2021-07-14
1. POC 진행전 사전 테스트 및 점검
2. AD 관련 성능 테스트


## 2021-07-13
1. 논문 읽기 (AD loss)
2. RAPP 추가 구현 및 성능 테스트
...
## 2021-07-09
1. 출장, conveyer belt 사러
2. RAPP(ord, sap, nap) 성능 테스트 비교 및 MLP 성능 테스트
...
1. 세미나 발표
2. C++ GUI 구현준비, show 함수 만들기



## 2021-06-17

1. C++ GUI 구현준비, show 함수 만들기
2. 논문 읽기
3. 세미나 준비
...
1. 성능 테스트를 위한 코드 구현(CAE with HSI data)
...
1. 파이토치 모델을 C++ 환경에서 돌리기위한 마이그레이션 테스트 및 메뉴얼 작성



## 2021-06-07

1. 파이토치 모델을 C++ 환경에서 돌리기위한 마이그레이션 테스트 



## 2021-06-06

1. torch script를 통한 모델 추출 및 C++ 에서 로드방법 공부
2. 논문 읽기 
...
1. MFC(win 32) study
2. 논문 구현모델 성능 개선 및 오픈데이터셋 테스트



## 2021-06-03

1. 논문 구현모델 성능 개선 및 오픈데이터셋 테스트
...
1. 프로그래머스 과제관 이미지 클래스 분류 모델 만들기
2. 논문 읽기
...
1. 알고리즘 및 SQL 풀이
2. 토이프로젝트 진행(노트북 도난방지 프로그램)
   - 기능별 모듈화 진행
   - Initializing, Thread ...etc..
...
1. 이력서 작성 및 제출
2. 알고리즘 풀이
3. 토이프로젝트 진행(노트북 도난방지 프로그램)
   - 전원 상태 여부에 따른 Slack 메시지 전송
   - 메인 틀 수정
...
1. C++ study
2. 데이콘 모션 키포인트 검출 AI 경진대회 베이스라인 체크
...
1. C++ study
2. 데이콘 모션 키포인트 검출 ai 경진대회 데이터 확인
